...
class ManageContactsFrame(Frame):
    ...
    def create_view_all_contacts_frame(self):
        ...
        self.contacts_treeview.column('email_id', width=225)
        self.contacts_treeview.column('city', width=125)
        self.contacts_treeview.grid(row=2, column=0, columnspan=2, pady=10)
        self.contacts_treeview.bind('<<TreeviewSelect>>', self.contacts_treeview_selection)
    ...
    def profile_pic_button_click(self):
        photo = Image.open(filedialog.askopenfilename())
        photo_name = self.name_entry.get().replace(" ", "")
        self.photo_path = "Photos\\"+photo_name+"."+photo.format
        photo.save(self.photo_path)

    def add_button_click(self):
        self.cur.execute("select * from Contact where EmailId = '{0}'".format(self.email_id_entry.get()))
        ...
        if row is None:
            ...
            self.add_new_contact_button.destroy()
            self.create_view_all_contacts_frame()
        else:
            messagebox.showerror("Error Message","Contact of {0} email id is already added".format(self.email_id_entry.get()))
    ...
    def contacts_treeview_selection(self,event):
        ...
        self.profile_pic = Label(self.update_delete_contact_frame, image=self.update_delete_contact_frame.image,
    ...
    def update_button_click(self):
        ...
        PhotoPath = '{4}' where EmailId = '{5}'""".format(
            self.name_entry.get(),
            self.phone_no_entry.get(),
            self.email_id_entry.get(),
            self.city_combobox.get(),
    ...
    def delete_button_click(self):
        if messagebox.askquestion("Confirmation Message","Are you sure to delete?") == 'yes':
            self.cur.execute("delete from Contact where EmailId = '{0}'".format(self.old_email_id))
            self.con.commit()
    ...
    def name_entry_key_release(self,event):
        self.fill_contacts_treeview("""select * from Contact 
        where name like '%{0}%'""".format(self.name_entry.get()))